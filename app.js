...
app.get('/',function(req,res){
  ...
  app.get('/home',async function(req,res){
      ...
      if(req.session.log){
        await db.collection('myCollection').findOne({username: req.session.username , password :req.session.password},function(err,user){
       if(err){
        console.log(err);
       }
       else {
  ...
  if(req.session.log){
   ...
   if(err){
    ...
    if(user){
      ...
      res.render('wanttogo',{wantToGoDestination : user.myWantToGoList});
    }
    else {
      res.redirect('login');
    }
...
app.post('/Search',async function(req,res){
  await db.collection("myCollection").findOne({username: req.session.username, password :req.session.password}, function(err, currentUser){
    if(err) {
      console.log(err);
    }
...
app.post('/login',async function(req,res){
  ...
  await db.collection('myCollection').findOne({username: userx},function(err,user){
    if(err){
      console.log(err);
    }
    else {
